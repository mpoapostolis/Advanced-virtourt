...
export function MobileSceneSelector() {
  const { data: items } = useItems();
  const router = useRouter();
  const { data: scenes } = useScenes();
  const { scene, pId } = router.query;
  return (
    <div className="fixed bottom-0 z-50 flex w-screen justify-center lg:hidden">
      <div className="  inline-block w-fit overflow-x-auto whitespace-nowrap rounded-t  bg-black pt-2 opacity-90  transition duration-200 ">
        {scenes.map((s) => (
          <Link
            ...
            role="button"
            className={clsx(
              "pointer-events-auto relative z-50 mx-2 inline-block aspect-auto  h-16 w-20  border border-base-300 bg-opacity-40",
              {
